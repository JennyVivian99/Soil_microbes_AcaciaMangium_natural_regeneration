...
ps_rarefiedMPJ <- rarefy_even_depth(ps_MainPJ, sample.size =   24803 , replace = FALSE, trimOTUs = TRUE, verbose = TRUE)
...
t1$cal_diff(method = "KW_dunn")
t1$data_stat
t1$plot_alpha(measure = "Observed", shape = "Landcover")+labs(title="Fungi ASVs richness")
# Further Post hoc analyses to implement if significant results from KW_dunn,
# for e.g. duncan.test
...
t1$cal_diff(method = "KW_dunn")
head(t1$res_diff)
t1$plot_alpha(measure = "Observed", shape = "Landcover")+labs(title="Fungi Phylum richness")
...
# t1 <- trans_alpha$new(dataset = meco_datasetMPJ, group = "Landcover")
# t1$cal_diff(method = "KW_dunn", formula = "Group+Type+Group:Type")
...
plot + labs(title="Fungi communities")+theme_bw()
...
t1 <- trans_abund$new(dataset = meco_datasetMPJ, taxrank = "Phylum", ntaxa = 5)
...
t1$plot_bar(others_color = "grey70", facet = "Landcover", xtext_keep = FALSE, legend_text_italic = FALSE)+
  labs(title = "Relative abundance of fungi phyla")
...
g1 + theme_classic() + theme(axis.title.y = element_text(size = 18))+labs(title = "Fungi phylum relative abundance")
# Mean at ASVs level
t1 <- trans_abund$new(dataset = meco_datasetMPJ, taxrank = "Species", ntaxa = 10, groupmean = "Landcover")
g1 <- t1$plot_bar(order_x = c("Grassland", "TwoYearsOld", "TenYearsold", "TwentyfourYearsold", "Remnant"), legend_text_italic = FALSE)
g1 + theme_classic() + theme(axis.title.y = element_text(size = 18))+ ylim(0,23)+labs(title = "Fungi ASVs relative abundance")
...
# show 15 taxa at ASVs level
t1 <- trans_abund$new(dataset = meco_datasetMPJ, taxrank = "Species", ntaxa = 15)
t1$plot_box(group = "Landcover", xtext_angle = 30)
# Heatmap
...
#Show the plot
g1
g1 + theme(axis.text.y = element_text(face = 'italic'))+labs(title = "Fungi genera relative abundance")
# Donut chart
...
t1 <- trans_venn$new(dataset = tmp, ratio = NULL)
t1$data_summary
t1$plot_venn()
...
t1$res_diff
write.table(t1$res_diff,"LefSeFungi.csv")
# At the genus level
...
soil_amp_network$Grassland$cal_network_attr()
soil_amp_network$Grassland$get_edge_table()
soil_amp_network$Grassland$res_edge_table
# select samples of "TwoYearsOld"
tmp <- clone(meco_datasetMPJ)
...
# select samples of "TwentyfourYearsold"
tmp <- clone(meco_datasetMPJ)
...
# Re-call the library needed
soil_amp_network %<>% cal_module(undirected_method = "cluster_fast_greedy")
soil_amp_network$Grassland$get_edge_table()
soil_amp_network$Grassland$res_edge_table

# Plots
soil_amp_network$Grassland$save_network(filepath = "GrasslandFungi.gexf")
# plot node roles with phylum information
soil_amp_network$Grassland$plot_taxa_roles(use_type = 2)
soil_amp_network$TwoYearsOld$save_network(filepath = "TwoYearsOldFungi.gexf")
# plot node roles with phylum information
soil_amp_network$TwoYearsOld$plot_taxa_roles(use_type = 2)
# Plot with connections
soil_amp_network$Grassland$cal_sum_links(taxa_level = "Phylum")
# interactive visualization; require chorddiag package; see https://github.com/mattflor/chorddiag
soil_amp_network$Grassland$plot_sum_links(plot_pos = TRUE, plot_num = 10, color_values = RColorBrewer::brewer.pal(10, "Paired"))
# From v1.2.0, method = "circlize" is available for conveniently saving the static plot
# If circlize package is not installed, first run: install.packages("circlize")
soil_amp_network$Grassland$plot_sum_links(method = "circlize", transparency = 0.2, annotationTrackHeight = circlize::mm_h(c(5, 5)))
soil_amp_network$TwoYearsOld$cal_sum_links(taxa_level = "Phylum")
# interactive visualization; require chorddiag package; see https://github.com/mattflor/chorddiag
soil_amp_network$TwoYearsOld$plot_sum_links(plot_pos = TRUE, plot_num = 10, color_values = RColorBrewer::brewer.pal(10, "Paired"))
# From v1.2.0, method = "circlize" is available for conveniently saving the static plot
# If circlize package is not installed, first run: install.packages("circlize")
soil_amp_network$TwoYearsOld$plot_sum_links(method = "circlize", transparency = 0.2, annotationTrackHeight = circlize::mm_h(c(5, 5)))
...
soil_amp_network$TwentyfourYearsold$cal_sum_links(taxa_level = "Phylum")
# interactive visualization; require chorddiag package; see https://github.com/mattflor/chorddiag
soil_amp_network$TwentyfourYearsold$plot_sum_links(plot_pos = TRUE, plot_num = 10, color_values = RColorBrewer::brewer.pal(10, "Paired"))
# From v1.2.0, method = "circlize" is available for conveniently saving the static plot
# If circlize package is not installed, first run: install.packages("circlize")
soil_amp_network$TwentyfourYearsold$plot_sum_links(method = "circlize", transparency = 0.2, annotationTrackHeight = circlize::mm_h(c(5, 5)))
soil_amp_network$Remnant$cal_sum_links(taxa_level = "Phylum")
# interactive visualization; require chorddiag package; see https://github.com/mattflor/chorddiag
soil_amp_network$Remnant$plot_sum_links(plot_pos = TRUE, plot_num = 10, color_values = RColorBrewer::brewer.pal(10, "Paired"))
# From v1.2.0, method = "circlize" is available for conveniently saving the static plot
# If circlize package is not installed, first run: install.packages("circlize")
soil_amp_network$Remnant$plot_sum_links(method = "circlize", transparency = 0.2, annotationTrackHeight = circlize::mm_h(c(5, 5)))

# Extract all the res_network_attr tables in the networks and merged them into one final table by using cal_network_attr function in meconetcomp package.
...
tmp2$otu_table
Intersec_all <- subset_network(soil_amp_network, venn = tmp2, name = "TwentyfourYearsold")
...
# Error tells that not negative relationships are present at phylum level
# Try at ASVs level
soil_amp_network_edgetax_negative <- edge_tax_comp(soil_amp_network, taxrank = "Species", label = "-", rel = TRUE)
...
t1$cal_ordination(method = "dbRDA", use_measure = "jaccard", group="Landcover")
...
g1+labs(title = "Fungi genera db-RDA")
...
g1 <- quickcor(t1$data_env, type = "upper", cor.test = TRUE, show.diag = FALSE) +
  ...
  anno_link(aes(colour = pd, size = rd), data = plot_table,label.size = 3.5) +
...
g1+ labs(title="Fungi phyla correlation with environmental parameters")
...
# Save the result
write.table(t2$res_spe_func_perc, "FunctionFungi.csv", row.names = TRUE)
write.table(t2$res_spe_func_raw_funguild, "FunctionFungiAll.csv")
...
t9<-read.csv("FunctionFungi_MultiPlot.csv", row.names=1)
...
# plot the PCoA result with confidence ellipse
t12$plot_ordination(plot_color = "Landcover",plot_type = c("point", "ellipse"))+theme_bw()+geom_text(aes(label = sample_id),vjust = -0.6)
...
t3$res_cor[t3$res_cor$Taxa=="Symbiotrophs",]
write.table(t3$res_cor, "CorrelationFunctionsEnv.csv", sep = "t")
# Visualise
g1<-t3$plot_cor(xtext_angle = 90)
g1+ labs(title="Fungi functions correlation with environmental parameters")
...
t2 <- trans_diff$new(dataset = tmp_mt, method = "KW_dunn", group = "Landcover", p_adjust_method="none", taxa_level = "func")
